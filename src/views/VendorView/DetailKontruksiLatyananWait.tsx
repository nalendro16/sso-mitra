...
export const DetailKontruksiLayananWait: React.FC = () => {
  ...
  return (
    <div>
      ...
      {dataDetailRAB?.isLoading ? (
        <div className='bg-gradient-header py-4 px-8 rounded-xl flex flex-col mt-4 mx-2'>
          <Skeleton height={14} width={120} baseColor='#90CDEB' />
          <Skeleton height={14} width={80} baseColor='#90CDEB' />
        </div>
      ) : (
        <div className='bg-gradient-header py-4 px-12 rounded-xl flex items-center gap-2 mt-4 mx-2'>
          <img src={images.ic_ask} alt='' className='mr-4' />
          <div className='text-sm'>
            <div className='text-white font-semi-bold text-base'>
              RAB sedang ditinjau
            </div>
            <div className='text-white mt-2'>
              Customer sedang meninjau usulan RAB, mungkin membutuhkan beberapa
              saat. Cek secara berkala untuk bisa mengerjakan tahap selanjutnya.
            </div>
          </div>
        </div>
      )}
